...
function readEntries(read: MemoryRead, address: bigint, count: number): Buffer {
...
function decodeEntries(bytes: Buffer, maximumId: number): (number | null)[] | null {
...
export function readProfileSpells(read: MemoryRead, base: bigint, profile: bigint): ObservedSpells {
  try {
    ...
    const bookBytes = readEntries(read, bookAddress, P.spellbookSlots)
    const gemBytes = readEntries(read, gemAddress, P.memorizedSpellSlots)
    const book = decodeEntries(bookBytes, manager.maximumId)
    const memorizedSpells = decodeEntries(gemBytes, manager.maximumId)
