...
interface SessionDeps { read: () => Promise<GearProgressionContext>; publish: (state: GearCharacterReading) => void; now?: () => number }
...
export function gearProgressionSession(deps: SessionDeps): { tick: () => void; invalidate: () => void; stop: () => void } {
