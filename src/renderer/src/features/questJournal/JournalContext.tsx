...
function ProfileCorrection({ context, mutate }: { context: QuestJournalContext; mutate: (action: JournalAction) => Promise<void> }): JSX.Element {
  ...
  const [classesDraft, setClassesDraft] = useState<string | null>(null)
  ...
  const useDetected = (): void => {
    setLevelDraft(null)
    setClassesDraft(null)
    void mutate({ action: 'profile', classes: [] })
  }
  return (
        ...
        <Stack spacing={1}>
          ...
          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
            <TextField size="small" type="number" label="Character level" value={level} sx={{ width: 130 }}
              slotProps={{ htmlInput: { min: 1, max: 125 } }} onChange={(event) => setLevelDraft(event.target.value)} />
            ...
            <Button size="small" disabled={!context.characterId} onClick={useDetected}>Use detected profile</Button>
          </Stack>
