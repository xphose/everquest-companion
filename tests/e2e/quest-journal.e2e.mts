...
async function references(page: Page): Promise<void> {
  ...
  await page.getByRole('button', { name: 'a gnoll brewer', exact: true }).first().click()
  await page.waitForSelector('[data-testid="mobs-back"]')
  check('Walkthrough People links open existing NPC details directly', (await shown(page, '[data-testid="mobs-back"]')).toLowerCase().includes('quest journal'))
  ...
  check('Walkthrough Items opens its selected source item directly', (await shown(page, '[data-testid="loot-detail-title"]')).includes('Blackburrow Cask'))
