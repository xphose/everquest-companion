...
interface NativeFixture { mode: MacroMode; classes: ClassAbbr[]; book: number[]; gems: (number | null)[]; reads: number }
...
function row(id: number, name: string, classColumn: number, effect: string): string {
  ...
  fields[30] = [2001, 3001, 3002].includes(id) ? '51' : '5'
...
export async function controlMacroWorker(app: ElectronApplication, root: string): Promise<void> {
  await app.evaluate((_electron, stagedRoot) => {
    ...
    state.macroFixture = { mode: 'live', classes: ['MAG', 'SHM'], book, gems: [...book, ...Array<null>(12).fill(null)], reads: 0 }
